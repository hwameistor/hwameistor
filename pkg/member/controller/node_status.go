...
func (m *manager) syncNodesStatus() {
	...
	for _, node := range nodeList.Items {
		...
		if !ok {
			...
			m.setNodeState(ctx, &node, udsv1alpha1.NodeStateOffline)
			...
			if int32(currTime.Sub(lease.Spec.RenewTime.Time).Seconds()) > *lease.Spec.LeaseDurationSeconds {
				...
				m.setNodeState(ctx, &node, udsv1alpha1.NodeStateReady)
			}
		}
	}
}

func (m *manager) setNodeState(ctx context.Context, node *udsv1alpha1.LocalStorageNode, newState udsv1alpha1.State) error {
	...
	if node.Status.State == udsv1alpha1.NodeStateMaintain && newState == udsv1alpha1.NodeStateReady {
		return nil
	}
