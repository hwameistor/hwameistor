...
var _ = ginkgo.Describe("localstorage volume test ", ginkgo.Label("periodCheck1"), func() {
	...
	ginkgo.Context("create 2 deployment", func() {

		ginkgo.It("create 2 deployment", func() {
			//create deployment
			...
			exampleDeployment2 := &appsv1.Deployment{
				ObjectMeta: metav1.ObjectMeta{
					Name:      utils.DeploymentName2,
					Namespace: "default",
				},
				Spec: appsv1.DeploymentSpec{
					Replicas: utils.Int32Ptr(1),
			...
			err = client.Create(ctx, exampleDeployment2)
			if err != nil {
				logrus.Printf("%+v ", err)
				f.ExpectNoError(err)
			}
		...
		ginkgo.It("deploy STATUS should be AVAILABLE", func() {
			...
			if err != nil {
				logrus.Infof("deployment1 ready timeout")
				logrus.Error(err)
			}
			gomega.Expect(err).To(gomega.BeNil())

			deployment2 := &appsv1.Deployment{}
			deployKey2 := ctrlclient.ObjectKey{
				Name:      utils.DeploymentName2,
				Namespace: "default",
			}
			err = client.Get(ctx, deployKey, deployment)
			...
			logrus.Infof("waiting for the deployment to be ready ")
			err = wait.PollImmediate(3*time.Second, framework.PodStartTimeout, func() (done bool, err error) {
				if err = client.Get(ctx, deployKey2, deployment2); deployment.Status.AvailableReplicas != int32(1) {
					return false, nil
				}
				return true, nil
			})
			if err != nil {
				logrus.Infof("deployment2 ready timeout")
				logrus.Error(err)
			}
			gomega.Expect(err).To(gomega.BeNil())
	...
	ginkgo.Context("Clean up the environment", func() {
		ginkgo.It("Delete test Deployment-1", func() {
			//delete deploy
			deployment := &appsv1.Deployment{}
			deployKey := ctrlclient.ObjectKey{
				Name:      utils.DeploymentName,
		...
		ginkgo.It("Delete test Deployment-2", func() {
			//delete deploy
			deployment := &appsv1.Deployment{}
			deployKey := ctrlclient.ObjectKey{
				Name:      utils.DeploymentName2,
