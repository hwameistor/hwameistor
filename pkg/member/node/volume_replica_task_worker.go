...
func (m *manager) processVolumeReplica(replicaName string) error {
	...
	if replica.Spec.Delete && replica.Status.State != udsv1alpha1.VolumeReplicaStateToBeDeleted && replica.Status.State != udsv1alpha1.VolumeReplicaStateDeleted {
	...
	if replica.Spec.Kind != udsv1alpha1.VolumeKindRAM && replica.Spec.Kind != m.storageMgr.NodeConfig().LocalStorageConfig.VolumeKind {
		replica.Status.State = udsv1alpha1.VolumeReplicaStateInvalid
		return m.apiClient.Status().Update(context.TODO(), replica)
	}

	switch replica.Status.State {
	case "", udsv1alpha1.VolumeReplicaStateInvalid:
		return m.processVolumeReplicaSubmit(replica)
	case udsv1alpha1.VolumeReplicaStateCreating:
		return m.processVolumeReplicaCreate(replica)
	case udsv1alpha1.VolumeReplicaStateReady, udsv1alpha1.VolumeReplicaStateNotReady:
		return m.processVolumeReplicaCheck(replica)
	case udsv1alpha1.VolumeReplicaStateToBeDeleted:
		return m.processVolumeReplicaDelete(replica)
	case udsv1alpha1.VolumeReplicaStateDeleted:
...
func (m *manager) processVolumeReplicaSubmit(replica *udsv1alpha1.LocalVolumeReplica) error {
	logCtx := m.logger.WithFields(log.Fields{"replica": replica.Name, "spec": replica.Spec})
	logCtx.Debug("Submit a VolumeReplica")

	replica.Status.State = udsv1alpha1.VolumeReplicaStateCreating
	return m.apiClient.Status().Update(context.TODO(), replica)
}

func (m *manager) processVolumeReplicaCreate(replica *udsv1alpha1.LocalVolumeReplica) error {
	logCtx := m.logger.WithFields(log.Fields{"replica": replica.Name, "spec": replica.Spec})
...
func (m *manager) processVolumeReplicaCheck(replica *udsv1alpha1.LocalVolumeReplica) error {
	logCtx := m.logger.WithFields(log.Fields{"replica": replica.Name, "spec": replica.Spec, "status": replica.Status})
	...
	if replica.Status.State == udsv1alpha1.VolumeReplicaStateReady &&
		replica.Spec.Kind == udsv1alpha1.VolumeKindLVM &&
		replica.Spec.RequiredCapacityBytes > replica.Status.AllocatedCapacityBytes+udsv1alpha1.VolumeExpansionCapacityBytesMin {
	...
	if err = m.configManager.EnsureConfig(testReplica); err != nil {
		...
		testReplica.Status.State = udsv1alpha1.VolumeReplicaStateNotReady
...
func (m *manager) processVolumeReplicaDelete(replica *udsv1alpha1.LocalVolumeReplica) error {
	logCtx := m.logger.WithFields(log.Fields{"replica": replica.Name, "spec": replica.Spec, "status": replica.Status})
	...
	newReplica.Status.State = udsv1alpha1.VolumeReplicaStateDeleted
	return m.apiClient.Status().Patch(context.TODO(), newReplica, patch)
}

func (m *manager) processVolumeReplicaCleanup(replica *udsv1alpha1.LocalVolumeReplica) error {
	logCtx := m.logger.WithFields(log.Fields{"replica": replica.Name, "spec": replica.Spec, "status": replica.Status})
