...
本地存储是 HwameiStor 的一部分，必须与本地磁盘管理器一起工作。建议用户 [通过 helm-charts 部署](../01.installation/01.helm-chart.md)。

## 独立安装部署方式

开发者可以[独立安装](../01.installation/01.helm-chart.md) local-storage，从源代码进行安装，主要用于开发、测试场景。对于这种安装方式，您需要事先安装好[本地磁盘管理器](02.local-disk-manager.md)。
