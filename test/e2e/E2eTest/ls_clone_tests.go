...
var _ = ginkgo.Describe("localstorage clone test ", ginkgo.Label("clone"), func() {
	...
	ginkgo.Context("create a clone PVC", func() {
		ginkgo.It("create a clone PVC", func() {
			//create PVC
			storageClassName := "local-storage-hdd-lvm"
	...
	ginkgo.Context("create a clone deployment", func() {

		ginkgo.It("create deployment", func() {
			//create deployment
			exampleDeployment := &appsv1.Deployment{
				ObjectMeta: metav1.ObjectMeta{
					Name:      utils.DeploymentName + "-clone",
					Namespace: "default",
				},
				Spec: appsv1.DeploymentSpec{
					Replicas: utils.Int32Ptr(1),
	...
	ginkgo.Context("Test the clone volume", func() {
		ginkgo.It("check the clone data", func() {
			logrus.Infof("check the clone data")
			config, err := config.GetConfig()
			if err != nil {
				return
			}
			...
			apps, err := labels.NewRequirement("app", selection.In, []string{"clone"})
			selector := labels.NewSelector()
			selector = selector.Add(*apps)
			listOption := ctrlclient.ListOptions{
				LabelSelector: selector,
	...
	ginkgo.Context("Clean up the environment", func() {
		ginkgo.It("Delete test Deployment", func() {
			...
			logrus.Infof("deleting clone Deployment ")
